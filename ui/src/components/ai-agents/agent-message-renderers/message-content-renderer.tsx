...
export function MessageContentRenderer({ message, agents }:MessageRendererProps) {
  ...
  const getMessageType = (): (AgentTask | string) => {
    ...
    if (message.agentId === AgentID.TOPOLOGY_DESIGNER && message.taskId) return message.taskId

    if (message.agentId === AgentID.LOG_SUMMARIZER && message.taskId) return message.taskId
